...
class FoodConsumptionEventsController < ApplicationController
  ...
  def create
    ...
    broadcast = broadcast_for(@food_consumption_event)
  ...
  def update
    respond_to do |format|
      if @food_consumption_event.update(food_consumption_event_params)
        Broadcasts::UpdateFoodConsumptionEvent.perform(@food_consumption_event)
        format.html do
          redirect_to food_consumption_event_url(@food_consumption_event),
                      notice: 'Food consumption event was successfully updated.'
        end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html do
        redirect_to food_consumption_events_url, notice: 'Food consumption event was successfully destroyed.'
      end
  ...
  def broadcast_for(food_consumption_event)
    food_consumption_event.new_record? ? Broadcasts::CreateFoodConsumptionEvent : Broadcasts::UpdateFoodConsumptionEvent
  end
